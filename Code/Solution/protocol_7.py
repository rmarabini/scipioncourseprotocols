...
class XmippProtABS(EMProtocol):
    ...
    def _validate(self):
        msg = []
        len1 = self.inputParticles1.get().getSize()
        len2 = self.inputParticles2.get().getSize()
        if len1 != len2:
            msg.append("length of the two data sets is different %d != %d"%
                       (len1, len2))
        x1, y1, n1 = self.inputParticles1.get().getDimensions()
        x2, y2, n2 = self.inputParticles2.get().getDimensions()
        if x1 != x2:
            msg.append("image size  is different %d != %d"%
                       (x1, x2))
        return msg
    ...
    def _summary(self):
        message= "Processed %d images" % \
                 self.inputParticles1.get().getSize()
        return [message]
