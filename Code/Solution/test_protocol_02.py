...
class TestXmippABS(BaseTest):
    ...
    def test1(self):
        ...
        protABS = self.newProtocol(XmippProtABS,
                                   objLabel='sum particles',
                                   )

        protABS.inputParticles1.set(inputParts)
        protABS.inputParticles2.set(outputParts)

        self.launchProtocol(protABS)

        outputParts2 = getattr(protPreprocess, 'outputParticles', None)

        self.assertIsNotNone(outputParts2)
        self.assertEqual(outputParts2.getSize(), inputParts.getSize())
